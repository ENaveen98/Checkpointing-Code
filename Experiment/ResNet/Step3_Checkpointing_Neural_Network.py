...
def memtest():
    ...
    class Segment_4(nn.Module):
        ...
        def forward(self, x):
            out = self.bn7(self.conv7(self.relu4(self.bn6(self.conv6(x))))) + self.downsample20(x) # If we need to add residual after some layers all the layers inbetween are considered as a single layer, hence the expression is in a single line
            out = self.relu4(out)
